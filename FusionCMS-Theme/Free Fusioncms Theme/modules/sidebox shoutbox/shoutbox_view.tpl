<script type="text/javascript">
	...
	var Shoutbox = {
		...
		submit: function()
		{
			...
			{
				UI.alert("Сообщение должно быть в диапазоне 0-255 символов!");
			}
			else
			{
				// Disable fields
				...
				$.post(Config.URL + "sidebox_shoutbox/shoutbox/submit", {message: message.val(), csrf_token_name: Config.CSRF}, function(data)
				{
					...
					$("#the_shouts").prepend('<div class="shout" id="my_shout_' + content.uniqueId + '" style="display:none">'+
												...
												'<div class="shout_author"><a href="' + Config.URL + 'profile/' + content.id + '" data-tip="Просмотр профиля">' + content.name + '</a> said:</div>' +
												content.message +
											'</div>');
...
<div id="shoutbox">
{if $logged_in == false}
	<form onSubmit="UI.alert('Войдите чтобы писать!');return false;">
		<textarea name="shoutbox_content" placeholder="Войдите чтобы писать!" disabled="disabled"></textarea>
		<div class="shout_characters_remaining"><span id="shoutbox_characters_remaining">0 / 255</span></div>
		<input type="submit" id="shoutbox_submit" value="Отправить"/>
		<div class="clear"></div>
	</form>
{else}
	<form onSubmit="Shoutbox.submit(); return false">
		...
		<div class="shout_characters_remaining"><span id="shoutbox_characters_remaining">0 / 255</span></div>
		<input type="submit" name="shoutbox_submit" value="Отправить" />
		<div class="clear"></div>
	</form>
{/if}
...
{if $count > 5}
	<div id="shoutbox_view">
		<a href="javascript:void(0)" onClick="Shoutbox.load(currentShout - shoutsPerPage)" id="shoutbox_newer" style="display:none;">&larr; Новые</a>&nbsp;
		&nbsp;<a href="javascript:void(0)" onClick="Shoutbox.load(currentShout + shoutsPerPage)" id="shoutbox_older">Старые &rarr;</a>
	</div>
{/if}
</div>
